...
\]
\begin{align*}
  E & : \mathbb{E} \to (\mathit{Id}\to \mathbb{D}) \to \mathcal{P}(\mathbb{D})\\
  E\, n \, \rho & = \{ N(n) \} \\
...
\end{align*}
\begin{align*}
  D^{*}_1 \cdot D^{*}_2 & \overset{\mathrm{def}}{=}
    \left\{ d' \middle|
    \begin{array}{l}
      \exists t d d_2.\, t \in D^{*}_1 \text{ and }  d_2 \in D^{*}_2, \\
      (d,d') \in t \text{ and } d \sqsubseteq d_2
    \end{array}
    \right\} \\[1ex]
  \uplambda^S & : (\mathbb{D} \to \mathcal{P}(\mathbb{D})) \to \mathcal{P}(\mathbb{D}) \\
  \uplambda^S f &\overset{\mathrm{def}}{=}
      \{ t \mid \forall (d,d') \in t, d' \in f\, d \}
\end{align*}
\end{minipage}
...
defined in terms of the auxilliary application operator $D^{*}_1 \cdot
D^{*}_2$, which applies all the tables in $D^{*}_1$ to all the elements
(arguments) in $D^{*}_2$. A naive definition of the application operator
...
\subsection{$\lambda$-models}


\begin{definition}
  A cpo $D$ is \emph{reflexive} if $[D \to D]$ is a retract of $D$.
  That is, there are continuous maps $F:D \to [D \to D]$
  and $G: [D \to D] \to D $ such that for any $f \in [D \to D]$,
  \[
    (F \circ G)\, f = f
  \]
\end{definition}
...
\ESEM{ x }\rho &= \rho(x) \\
\ESEM{ (\LAM{x} e) }\rho &= 
  \{ (D,d') \mid  d' \in \ESEM{ e }[x\mapsto D]\rho \} \\
\ESEM{ (e_1\,e_2) }\rho &= \left\{ d' \, \middle| 
   \begin{array}{l}
   \exists D.\, (D,d') \in \ESEM{ e_1 }\rho \text{ and } D \subseteq \ESEM{ e_2 }\rho 
   \end{array}
\right\} \\[1ex]
   - \cdot - &: D_A \to D_A \to D_A \\
   D^{*}_1 \cdot D^{*}_2 &= \{ d' \mid \exists D. D \subseteq D^{*}_2 \land (D,d') \in D^{*}_1 \} \\[1ex]
   \uplambda^G &: (\mathcal{P}_f(B) \to D_A) \to D_A \\
   \uplambda^G f &= \{ (D,d') \mid D \in \mathcal{P}_f(D) \land d' \in f\, D \} 
\end{align*}
...
\begin{theorem}
  ...
  y$ and $G= \uplambda f.\, \uplambda^G f$. Therefore $D_A$ is a
  $\lambda$-model.
\end{theorem}
\begin{proof}
  ...
  \begin{align*}
    (F \circ G)\,f &= \uplambda x. (\uplambda^G f) \cdot x\\
...
\mathcal{E}_S & : \mathbb{E} \to (\mathit{Id} \to \mathcal{P}(\mathit{Nat})) \to \mathcal{P}(\mathit{Nat}) \\
\mathcal{E}_S\,x\,\rho &= \rho\,x \\  
\mathcal{E}_S\,(\LAM{x}e)\,\rho &= \{ \langle n, m \rangle \mid m \in \mathcal{E}_S\,e\,[x\mapsto e_n]\rho \} \\
\mathcal{E}_S\,(e_1\,e_2)\,\rho &= \{ m \mid \exists e_n. \, 
   \langle n,m \rangle \in \mathcal{E}_S\,e_1\,\rho
   \text{ and } e_n \subseteq \mathcal{E}_S\,e_2\,\rho \}\\
\end{align*}
