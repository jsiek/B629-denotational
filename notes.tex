...
\begin{gather*}
  ...
  \text{($\beta$)}  \vdash (\LAM{x}e_1)\APP e_2 = [x:=e_2]e_1 \\[2ex]
  \text{($\beta_v$)}  \vdash (\LAM{x}e)\APP v = [x:=v]e
\end{gather*}
...
\begin{definition}[$\lambda$-models]
  ...
  D, \cdot, M\rangle$ such that $D$ is a set, $\_\cdot\_:D \times D
  \to D$, and $M : \mathbb{E}\to(\mathbb{X}\pto D)\to D$, and the
  following conditions hold.
  \begin{enumerate}
  \item $M\, x\,\rho = \rho(x)$,
  \item $M\,(e_1\APP e_2)\,\rho = (M\,e_1\,\rho) \cdot (M\,e_2\,\rho)$ \label{cond:app},
  \item if $(\LAM{x}e) \APP e'$ is a $R$-redex, $(M\,(\LAM{x}e)\,\rho) \cdot (M\,e'\,\rho) = M\,e\,[x\by M\,e'\,\rho]\rho$ \label{cond:R},
  \item $\forall x \in \mathrm{FV}(e)$, if  $\rho(x) = \rho'(x)$,
       then $M\,e\,\rho = M\,e\,\rho'$.
  \item If $\forall d \in D, M\,e\,[x\by d]\rho = M\,e'\,[x\by d]\rho$,
       then $M\,(\LAM{x} e)\,\rho = M\,(\LAM{x} e')\,\rho$. \label{cond:lam}
  \end{enumerate}
\end{definition}
