...
\begin{align*}
  ...
  I\,(e_1\,e_2)\,\varrho &=  I\,e\,[x\mapsto v]\varrho'\\
      & \quad \text{if } I\,e_1\,\varrho = \langle \LAM{x}e,\varrho' \rangle \text{ and}\\
      & \quad I\,e_2\,\varrho = v
\end{align*}
...
\begin{definition}[$\lambda$-models]
  An $R$-model of the $\lambda$-calculus is a triple $\langle D, \cdot,
  E\rangle$ such that $D$ is a set, $\_\cdot\_:D \times D \to D$,
  and $E : \mathbb{E}\to(\mathbb{X}\to D)\to D$, and 
  \begin{enumerate}
  \item $E\, x\,\rho = \rho(x)$,
  \item $E\,(e_1\APP e_2)\,\rho = (E\,e_1\,\rho) \cdot (E\,e_2\,\rho)$,
  \item if $(\LAM{x}e) \APP e'$ is a $R$-redex, $(E\,(\LAM{x}e)\,\rho) \cdot (E\,e'\,\rho) = E\,e\,[x\by E\,e'\,\rho]\rho$,
  \item $\forall x \in \mathrm{FV}(e)$, if  $\rho(x) = \rho'(x)$,
       then $E\,e\,\rho = E\,e\,\rho'$.
     \item If $\forall d \in D, E\,e\,[x\by d]\rho = E\,e'\,[x\by d]\rho$,
       then $E\,(\LAM{x} e)\,\rho = E\,(\LAM{x} e')\,\rho$.
  \end{enumerate}
\end{definition}
...
\subsection{Filter models of $\lambda$-calculus}
...
\begin{definition}
  A \emph{filter $\lambda$-structure} is $\langle \mathbb{F}, \cdot, F\rangle$
  where 
\begin{align*}
  ...
  F &: \mathbb{E} \to (\mathbb{X} \to \mathbb{F}) \to \mathbb{F}\\
  F\,e\,\rho &\defeq \{ A \mid \exists \Gamma \models \rho, \Gamma \vdash e : A\} 
\end{align*}
\end{definition}
