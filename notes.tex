...
\subsection{Completeness wrt. Reduction Semantics}
...
$\lambda$-calculi. Here we focus on just one of them, the
$\mathcal{EHR}$ system for the call-by-value
$\lambda$-calculus~\citet{Egidi:1992aa}. Figure~\ref{fig:intersection-types}
defines types and the subtyping and type equivalence rules.  The
intuition behind the typing rules is that we can think of types as
sets of values, subtyping acts like set inclusion ($\subseteq$), and
intersection is just set intersection. Then, for example, the rule
(incl-L) corresponds to an obviously true statement about sets, that
$A \cap B \subseteq A$.
