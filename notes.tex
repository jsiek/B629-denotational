...
f = (\MLAM{d} \hat{e}_{1}(\hat{p}_0 \circ (\MLAM{d'}d) \circ \hat{e}_0)  \sqcup \hat{e}_{2}(\hat{p}_1 \circ (\MLAM{d'}d) \circ \hat{e}_1) \sqcup \cdots )\\
  ...
  \item $(\LAM{x}x) \APP (\LAM{x}x)$
    \begin{align*}
      ...
      &= (\bot, d_1, d_{002}, \ldots) \sqcup \cdots 
    \end{align*}
    It is a little troubling that the answer does not yet look the
    same as $\SEM{\LAM{x}x}$. This is probably because we would need
    to go further out to get an more accurate result.
