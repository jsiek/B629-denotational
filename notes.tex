...
isomorphic to its space of continuous functions:
\[
  D_\infty \cong [D_{\infty} \to D_{\infty}]
\]
The bijection will be witnessed by the following two functions.
\begin{align*}
 \mathit{up} &: D_{\infty} \to [D_{\infty} \to D_{\infty}] \\
 \mathit{down} &: [D_{\infty} \to D_{\infty}] \to D_{\infty}
\end{align*}
...
\]
So $(d_0,d_1,\ldots) \in D_\infty$
if $d_0 \in D_0$, $d_1 \in D_1$, etc.
The $D_i$'s are defined inductively as follows,
where $\to_c$ is the type of continuous functions.
\begin{align*}
  D_0 &= \{ \bot, * \} \\
  D_{n+1} &= D_n \to_c D_n
\end{align*}
...
continuous.)  The idea is that, as we go higher, the $D_i$'s provide
better approximations to all the continuous functions.
...
\begin{align*}
  e_i &: [D_i \to D_{i+1}] \\
  e_0 (d_0) &= \lambda y.\, d_0 \\
  e_{n+1}(d_{n+1}) &= e_n \circ d_{n+1} \circ p_n \\
  \\
  p_i &: [D_{i+1} \to D_i]  \\
  p_0(d_1) &= d_1(\bot) \\
  p_{n+1} &: [D_{n+2} \to D_{n+1}]  \\
  p_{n+1}(d_{n+2}) &= p_n \circ d_{n+2} \circ e_n
\end{align*}
...
So we can now get back to defining the up and down functions.  We
define $\mathit{up}(d)(x)$ to be the infinite tuple $y$ such that
$y_i$ is the join of applying $d_{j+1}$ to $x_j$, for all $j \geq
i$.
...
\begin{align*}
 \mathit{down} &: [D_{\infty} \to D_{\infty}] \to D_{\infty}\\
 \mathit{down}(f) &= (d_0, d_1, \ldots, d_n, \ldots) \\
 & \text{where } \\
 & d_0 = f(\bot) \\
 & d_{n+1} = \hat{p}_n \circ f \circ \hat{e}_n
\end{align*}

Where the $\hat{p}_m$ and $\hat{e}_m$ operations project and embed
between $D_\infty$ and $D_m$.  The $\hat{p}_m$ operation is simple, it
just selects the $m$th element from the infinite tuple.
\begin{align*}
 \hat{p}_m &: D_\infty \to D_m \\
 \hat{p}_m &= \pi_m
\end{align*}
The idea with $\hat{e}_m$ is that we take an element $d \in
D_m$ and create an infinite number of duplicates to fill in the output
tuple $(d_0, d_1, \ldots, d_n, \ldots)$. For $n \leq m$, we slide the
duplicate ``down'' by applying the appropriate number of projections.
For $n > m$, we slide the duplicate ``up'' by applying the appropriate
number of embeddings.

\begin{align*}
 \hat{e}_m &: D_m \to D_\infty \\
 \hat{e}_m(d) &= (d_0, d_1, \ldots, d_n, \ldots) \\
  & \text{where } d_n = 
  \begin{cases}
    (p_n \circ p_{n+1} \circ \cdots)(d) & \text{if } n \leq m \\
    (e_{n-1} \circ e_{n-2} \circ \cdots)(d) & \text{if } n > m
  \end{cases}
\end{align*}
...
\begin{align*}
  \SEM{x}\rho &= \rho(x) \\
  \SEM{e_1 \APP e_2}\rho &= \mathit{up}(\SEM{e_1}\rho)(\SEM{e_2}\rho) \\
  \SEM{\LAM{x} e}\rho &= \mathit{down}(\MLAM{d} \SEM{e}\rho(x:=d))
\end{align*}
