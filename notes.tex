...
\begin{align*}
  ...
  D_{n+1} &= D_n \to_c D_n
\end{align*}
For $D_0$, we have $\bot \sqsubseteq *$.  For the functions in
...
example, the function $\{ \bot \mapsto *, * \mapsto \bot \}$ is not
  ...
  \begin{tabular}{p{0.5in}p{0.5in}c}
   ...
   &
    ...
    \xymatrix@=10pt{
          & d_{222} \ar@{-}[d]\\
          & d_{122} \ar@{-}[dl] \ar@{-}[dr]\\
      d_{022} \ar@{-}[dr]&     & d_{112} \ar@{-}[dl] \ar@{-}[dr]\\
...
this by first mapping an input argument from $D_n$ to $D_\infty$,
applying $f$, and then mapping back from $D_\infty$ to $D_n$ to
produce a result. This mapping back and forth to $D_\infty$ is handled
by auxiliary functions $\hat{p}_n$ and $\hat{e}_n$.
\begin{align*}
 \mathit{abstr} &: (D_{\infty} \to_c D_{\infty}) \to D_{\infty}\\
...
\end{align*}
% to do: explain join and the order structure -Jeremy
...
\begin{Exercise}
  Compute the first three items in the tuple ($D_0 \times D_1 \times
  D_2 \times \cdots$) for the meaning of:
  \begin{itemize}
  \item $\LAM{x}x$
  \item $\LAM{x}\LAM{y}x$
  \end{itemize}
...
\begin{Answer}
  ...
  \item $\LAM{x}x$ 
    \begin{align*}
      ...
      &= \mathit{abstr}(\MLAM{d} d) \\
      &= (\bot, d_1, d_{012}, \ldots) 
    \end{align*}
    Also, we check that projection maps each element to its predecessor.
    \begin{align*}
      p_0(d_1) &= d_1(\bot) = \bot \\
