...
the $\lambda$-calculus.  As a secondary feature, we include
...
\begin{align*}
  ...
  E\, x \,\rho &= \{ \rho\,x \} \\
  E\, (\LAM{x}e)\,\rho &= 
      \{ t \mid \forall (d,d') \in t, d' \in E\,e\,[x\mapsto d]\rho) \} \\
...
\subsection{Implementation of the $\lambda$-calculus}
...
\begin{definition}[$\lambda$-models]
  ...
  \item if $(\LAM{x}e) \APP e'$ is a $R$-redex, $\SEM{(\LAM{x}e)}\rho \cdot \SEM{e'}\rho = \SEM{e}[x\by \SEM{e'}\rho]\rho$ \label{cond:R}, \\
        alternative: $\SEM{(\LAM{x}e)}\rho \cdot d = \SEM{e}[x\by d]\rho$ \label{cond:lam-app}
  \item $\forall x \in \mathrm{FV}(e)$, if  $\rho(x) = \rho'(x)$,
       then $\SEM{e}\rho = \SEM{e}\rho'$. \label{cond:env}
  \item If $\forall d \in D, \SEM{e}[x\by d]\rho = \SEM{e'}[x\by d]\rho$,
...
\begin{example}
  ...
  environment parameter maps variables to $\mathbb{D}$ instead of
  $\mathcal{P}(\mathbb{D})$. However, this difference can be bridged
  with the following alternative semantic function.
  \begin{align*}
    E' &: \mathbb{E} \to (\mathbb{X}\pto \mathcal{P}(\mathbb{D}))
         \to \mathcal{P}(\mathbb{D}) \\
    E'\,e\,\rho' &=
    \{ d \mid \exists \rho, d \in E\,e\,\rho \text{ and }
        \forall x \in \mathrm{dom}(\rho), \rho(x) \in \rho'(x) \}
  \end{align*}
  The triple $\langle \mathcal{P}(\mathbb{D}), \cdot, E')$ is a
  $\beta_v$-model of the $\lambda$-calculus.
  TODO: Prove it!
\end{example}
...
\begin{proof}
  \begin{align*}
    \SEM{e(x)} [x\by d]\rho &= \SEM{\LAM{x}e(x)}\rho \cdot d & \text{by condition \ref{cond:lam-app}}\\
       &= \SEM{\LAM{y}e(y)}\rho \cdot d & \text{$\alpha$ conversion}\\
       &= \SEM{e(y)} [y\by d]\rho & \text{by condition \ref{cond:lam-app}}
  \end{align*}
\end{proof}
...
\label{lem:subst-model}
  ...
  \item If $z \notin \mathrm{FV}(e)$, then
      $\SEM{ [x\by z]e } \rho = \SEM{e}[x\by \SEM{z}\rho]\rho$.
  \item If $\SEM{ [x\by e']e} \rho = \SEM{e}[x\by \SEM{e'}\rho]\rho$, then
      $\SEM{ ([x\by e']\LAM{y}e)} \rho = \SEM{(\LAM{y}e)}[x\by \SEM{e'}\rho]\rho$.
  \item $\SEM{ [x\by e']e} \rho = \SEM{e}[x\by \SEM{e'}\rho]\rho$.
  \end{enumerate}
\end{lemma}
\begin{proof}\ 
\begin{enumerate}
\item Assume $z \notin \mathrm{FV}(e)$. Let $e \equiv e(x)$.
  ...
  \end{align*}
\item TODO
\item  By induction on $e$. TODO
\end{enumerate}
\end{proof}

\begin{theorem}[Soundess of R-models wrt. $\lambda$ theory]
  Suppose $\langle D,\cdot,M\rangle$ is an $R$-model of the
  $\lambda$-calculus.
  \[
  \text{if } R \vdash e = e', \text{ then } \forall \rho, M\,e\,\rho = M\,e'\,\rho
  \]
\end{theorem}
\begin{proof}
  The proof is by induction on $R \vdash e = e'$.
  \begin{itemize}
  \item[Case (refl):] We have $\forall \rho, M\,e\,\rho = M\,e\,\rho$
    because $M$ is a function.
    
  \item[Case (sym):]
    By the IH, we have $\forall \rho, M\,e_1\,\rho = M\,e_2\,\rho$.
    So it trivially follows that $\forall \rho, M\,e_2\,\rho = M\,e_1\,\rho$.
    
  \item[Case (trans):]
    By the IH, we have $\forall \rho, M\,e_1\,\rho = M\,e_2\,\rho$
    and $\forall \rho, M\,e_2\,\rho = M\,e_3\,\rho$.
    Thus, we immediately have $\forall \rho, M\,e_1\,\rho = M\,e_3\,\rho$.
    
  \item[Case (cong):]
    Let $\rho$ be an arbitrary environment.
    We need to show that $M\,(e_1\APP e_2)\,\rho = M\,(e_3\APP e_4)\,\rho$.
    \begin{align*}
      M\,(e_1\APP e_2)\,\rho &= (M\,e_1\,\rho) \cdot (M\,e_2\,\rho) & \text{condition \ref{cond:app}}\\
        &= (M\,e_3\,\rho) \cdot (M\,e_4\,\rho) & \text{by IH}\\
        &= M\,(e_3\APP e_4)\,\rho & \text{condition \ref{cond:app}}
    \end{align*}
    
  \item[Case $\xi$:]
    We need to show $\forall \rho, M\,(\LAM{x}e)\,\rho = M\,(\LAM{x}e')\,\rho$.
    Let $\rho$ be an arbitrary environment.
    By condition \ref{cond:lam} it suffices to show that
    $M\,e\,[x\by d]\rho = M\,e'\,[x\by d]\rho$,
    and that follows from the IH.

  \item[Case $R$:]
    Let $\rho$ be an arbitrary environment.
    We need to show that
    $M\,((\LAM{x}e)\APP e')\,\rho = M\,([x\by e']e)\,\rho$,
    where $((\LAM{x}e)\APP e')$ is an $R$-redex.
    \begin{align*}
      M\,((\LAM{x}e)\APP e')\,\rho &=
      (M\,(\LAM{x}e)\,\rho) \cdot (M\,e'\,\rho) & \text{condition \ref{cond:app}} \\
      &= M \,e\, [x\by M\,e'\,\rho]\rho & \text{condition \ref{cond:R}} \\
      &= M\,([x\by e']e)\,\rho & \text{by Lemma~\ref{lem:subst-model}}
    \end{align*}
...
Our type constants are $\{\NAT,\nu\}$, so our types correspond to
$\mathbb{T}(\{\NAT,\nu\})$ of \citet{Alessi:2006aa} and the types of
\citet{Egidi:1992aa}.

Figure~\ref{fig:intersection-types} also defines the subtyping and
...
\[
\begin{array}{lrl}
  \text{types} & A,B,C \in \mathbb{T} ::= & \NAT \mid \nu \mid A \to B \mid A \cap B
\end{array}
\]
\[
\begin{array}{rlrl}
...
\caption{Intersection Type System}
\label{fig:intersect-type-system}
\end{figure}

Figure~\ref{fig:intersect-type-system}
