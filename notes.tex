...
\newcommand{\PSEM}[1]{\mathcal{E}_P\,#1\,}


\begin{figure}
\begin{align*}
  T^{*}_C &= C + \mathcal{P}_f(T^{*}_C) \times \mathcal{P}_f(T^{*}_C) \\
  D & \in \mathcal{P}_f(T^{*}_C) \\[2ex]
%% \PSEM{ n }\rho &= \{ n \} \\
%% \PSEM{ e_1 \oplus e_2 }\rho &= \{  n_1 \oplus n_2 \mid 
%%    n_1 \in \PSEM{ e_1 }\rho \land n_2 \in \PSEM{ e_2 }\rho \} \\
\mathcal{E}_{P}&: \mathbb{E} \to (\mathit{Id}\to\mathcal{P}_f(T^{*}_C)) \to \mathcal{P}(T^{*}_C)\\
\PSEM{ x }\rho &= \rho(x) \\
\PSEM{ (\LAM{x} e) }\rho &= 
...
\newcommand{\ESEM}[1]{\mathcal{E}_E\, #1 \,}



\subsection{The $\mathcal{P}(\omega)$ model}
\label{sec:p-omega}
