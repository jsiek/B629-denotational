...
holds, i.e. if two functions map equal arguments to equal results,
then the two functions are the same function.
\end{definition}

\begin{lemma}[Term Model is a pre-frame]
...
: A \in \Gamma$ to the set $T\,A$. A substitution $\sigma$ maps each
variable to a single expression. A substitution $\sigma$ represents
$\rho$ as follows.
\[
...
\[
  T\,e\,\rho \defeq [\sigma(e)] \text{ for some } \Gamma;\rho \vdash \sigma
\]
