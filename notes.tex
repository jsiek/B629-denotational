...
The second parameter is a type environment drawn from $E\,\Gamma$
and the result is in $E\,A$.  The definition of the semantic
...
$E\,A$ to the element produced by the body $e$ of the $\lambda$,
...
\[
  \rho \models \Gamma \defeq \forall x:A \in \Gamma, \rho(x) \in M\,A
\]
\end{definition}
...
\begin{definition}
A structure $(M,\cdot)$ is a \emph{pre-frame} when extensionality
...
\begin{definition}
  Suppose $(M_1,\cdot)$ and $(M_2,\cdot)$ are frames.
  A \emph{partial homomorphism} $h : M_1 \to M_2$
  is a type-indexed set of partial functions $h_A : M_1\,A \rightharpoonup M_2\,A$
  such that for every $A,B$ and $f \in M_1(A \to B)$,
  either
  \begin{itemize}
  \item there exists $g \in M_2(A\to B)$ such that $h_{A\to B}(f) = g$
    and for all $x$ in the domain of $h_A$
    \[
    h_B( f \cdot x ) = g \cdot h_A(x)
    \]
