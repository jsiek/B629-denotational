...
\begin{theorem}[Term Model is Sound and Complete wrt. Equational Theory]\ \\
  Suppose $\Gamma \vdash e : A$ and $\Gamma \vdash e' : A$.\\
...
\begin{proof}
  ...
  \begin{itemize}
  \item $\vdash e = e' \implies (T,\cdot) \models e = e'$ \\
    The term model is a frame (Lemma~\ref{lem:term-frame}),
    and frames are sound (Lemma~\ref{lem:frame-sound}).
  \item $(T,\cdot) \models e = e' \implies \vdash e = e'$ \\
    Wlog. we assume $\Gamma \subset \mathcal{G}$.
    Let $\rho$ be the identity environment $\{ x \mapsto [x] \mid x
    \in \mathrm{dom}(\Gamma) \}$. So we have $\rho \models \Gamma$
