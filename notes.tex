...
\end{lemma}
\begin{proof}
  Let $\rho'$ be an environment s.t. $\rho' \models \Gamma$.
  We need to show that $M_2\,e\,\rho' = M_2\,e'\,\rho'$.
  Define $\rho(x) = h^{-1}(\rho'(x))$, so $\rho'(x) = h(\rho(x))$.
  ($h$ is a surjection, so $h^{-1}$ is total.)
  \begin{align*}
    M_2\,e\,\rho' 
