...
\begin{proof}
  ...
  undefined.
  \marginnote{This is rather hand-wavy! --Jeremy}
...
%%  (todo: What about filter models? And isn't
...
%
%% Because we represent queries about functions as finite graphs, the
%% user of the semantics can only ask a finite number of questions about
%% a function per query.\footnote{If the user has an infinite number of
%%   questions, then they must have the patience to submit an infinite
%%   number of queries.}
...
\begin{gather*}
  ...
  %%           {\rho \vdash \IF{e_1}{e_2}{e_3} \Rightarrow d}
  %% \quad
...
%
%% To enable trying all possible finite subsets of a function's graph,
%% the semantics is set-valued. That is, the semantic function $E$ maps
%% programs to sets of elements, instead of a single element. In the case
%% when an expression produces a natural number, the set is just a
%% singleton of that number. However, when the expression produces a
%% function, the set contains all finite approximations of the function.
%
...
%% A naive definition of the application operator is given below. The
%% idea is to collect up all the results $d'$ that come from matching an
%% argument $d \in D_2$ with an entry $(d,d')$ from a table $t \in D_1$.
%% \begin{align*}
%%   D_1 \cdot D_2 &= 
%%   \{ d' \mid
...
%% \in D_2$, we require $d \sqsubseteq d_2$ instead of $d = d_2$.
...
%%         alternative:
%% if $(\LAM{x}e) \APP e'$ is a $R$-redex, $\SEM{(\LAM{x}e)}\rho \cdot \SEM{e'}\rho = \SEM{e}\ext{x}{\SEM{e'}\rho}{\rho}$\label{cond:R}, \\
...
%% \begin{proof}
%%   TODO
%%   This is where we probably use the requirements of
...
%% \mathit{set}(n) &= \{ k_0, k_1, \ldots, k_{m-1} \} \quad \text{where }
...
\end{cases}\\
%% \langle \rangle &= 0 \\
  %% \langle s, n \rangle &= (\langle s \rangle, n) \\
...
Resources: \citet{Scott:1971aa}, \citet{Stoy:1977aa}, \citet{Amadio:1998fk}

The goal here is to construct a domain $D_\infty$ such that it is
isomorphic to its space of continuous functions ($\to_c$).
\[
...
\]
So $(d_0,d_1,\ldots) \in D_\infty$ if $d_0 \in D_0$, $d_1 \in D_1$,
etc.  The idea is that, as we go higher, each $d_i$ in the tuple
...
D_1 =  \left\{
\begin{array}{c}
d_{12}=\{\bot \mapsto *, * \mapsto * \}, \\
d_{11}=\{\bot \mapsto \bot, * \mapsto * \},\\
d_{10}=\{\bot \mapsto \bot, * \mapsto \bot \}
\end{array}
\right\}
\]
(The function $\{ \bot \mapsto *, * \mapsto \bot \}$ is not
continuous.)  

To define $\mathit{apply}$ and $\mathit{abstr}$, we will need a way to
map elements from $D_i$ to $D_{i+1}$ and vice versa. That is, we shall
define an embedding-projection pair, $e_n$ and $p_n$, between each
neighboring domain $D_n$ and $D_{n+1}$.
\[
\xymatrix{
  D_0 \ar@/^/[r]^{e_0} & D_1 \ar@/^/[l]^{p_0} & \cdots &
  D_n \ar@/^/[r]^{e_n} & D_{n+1} \ar@/^/[l]^{p_n} & \cdots
}
\]
We define $e_i$ and $p_i$ by induction on $i$. The embedding function
at $i > 0$, that is, $e_{n+1}$, maps a function $d : D_{n+1} = (D_n
\to_c D_n)$ to a function at the next higher domain, i.e., $d' :
D_{n+1} \to_c D_{n+1}$. This is accomplished by projecting from
$D_{n+1}$ to $D_n$, applying $d$, and then embedding from $D_n$ to
$D_{n+1}$, as shown in the diagram below.
...
\begin{minipage}{0.5\textwidth}
\xymatrix{
 D_n \ar@{=>}[d]^{d} & D_{n+1} \ar[d]^{d'} \ar@{=>}[l]^{p_n} \\
 D_n \ar@{=>}[r]^{e_n} & D_{n+1} 
}
\end{minipage} 
\end{center}

The projection function $p_{n+1}$ maps $d : D_{n+2} = (D_{n+1} \to_c
D_{n+1})$ to a function $d' : D_{n+1} = (D_n \to_c D_n)$. This is
...
\xymatrix{
 D_n  & D_{n+1}  \ar@{=>}[l]^{p_n} \\
 D_n \ar[u]^{d'} \ar@{=>}[r]^{e_n} & D_{n+1} \ar@{=>}[u]^{d}
}
\end{minipage} 
\end{center}

For example, here is $e_0$ and $p_0$.
\begin{align*}
  e_0(\bot) &= \LAM{y} \bot = d_{10} \\
  e_0(*) &= \LAM{y} * = d_{12}\\
  \\
  p_0(d_{10}) &= d_{10}(\bot) = \bot \\
  p_0(d_{11}) &= d_{11}(\bot) = \bot \\
  p_0(d_{12}) &= d_{12}(\bot) = *
\end{align*}
...
%% Recall that a lattice $(D,\sqsubseteq)$ is a partial order with all
%% meets and joins.

%% A subset $Z$ of $D$ is \emph{directed} is every finite subset of $Z$
%% has an upper bound in $Z$.

%% An element $x\in D$ is \emph{well below} $y$ iff $y \sqsubseteq \sqcup
%% Z$ implies $x \subseteq z$ for some $z \in Z$, where $Z$ is a directed
%% subset of $D$.

%% A continuous lattice is one in which every element $y$ is the lub of
%% all its essential parts, that is, all of the elements that are well
%% below $y$.
%% \[
%%    y = \sqcup \{ x \mid x \ll y \}
%% \]

%% Let $D_0$ be a continuous lattice with at least two elements.
%% As we go to higher $D_n$'s, we are able to construct better
%% approximations of continuous functions.

%% We choose $D_0 = \{ \bot, *\}$. We shall define a family of
%% functions that map back and forth between each $D_n$ to $D_{n+1}$,
%% that is embedding-projection pairs.

%% \[
%% \xymatrix{
%%  D_0 \ar[d]^{d_1} & D_1 \ar[d]^{d_2} \ar[l]^{p_0}& \cdots & D_n \ar[d]^{d_{n+1}} & D_{n+1} \ar[d]^{d_{n+2}} \ar[l]^{p_n} & D_{n+2} \ar[l]^{p_{n+1}}& \cdots \\
%%  D_0 \ar[r]^{e_0} & D_1 & \cdots & D_n \ar[r]^{e_n} & D_{n+1} \ar[r]^{e_{n+1}} & D_{n+2} & \cdots
%% }
%% \]
...
%%    p \circ e = id
%%    \qquad
%%    e \circ p \sqsubseteq id
%% \]


%% $D_{\infty}$ is the inverse limit of this sequence, which means that
%% each element of $D_{\infty}$ is an infinite tuple of functions.
%% \[
%%   (d_1, d_2, d_3, \ldots )
%% \]
...
$d_{n+1}$ approximates $f$ in the domain $D_n$.  We shall accomplish
this by first mapping from $D_n$ to $D_\infty$, applying $f$, and then
mapping back from $D_\infty$ to $D_n$. This mapping back and forth to
$D_\infty$ will be handled by auxiliary functions $\hat{p}_n$ and
$\hat{e}_n$.
\begin{align*}
 \mathit{abstr} &: (D_{\infty} \to_c D_{\infty}) \to D_{\infty}\\
 \mathit{abstr}(f) &= (d_0, d_1, \ldots, d_n, \ldots)  \text{ where } \\
 & \qquad d_0 = f(\bot) \\
...
\begin{align*}
 ...
 \hat{e}_n(d) &= (d_0, d_1, \ldots, d_i, \ldots) \text{ where} \\
  & \qquad d_i = 
  \begin{cases}
    (p_i \circ p_{i+1} \circ \cdots \circ p_{n-1})(d) & \text{if } i \leq n \\
...
a $d \in D_\infty$ and an $x\in D_\infty$, and needs to return an
element of $D_\infty$ that represents the result of applying $d$ to
$x$ (as in function application). But recall that $d$ and $x$ are
infinite tuples of approximations and that the result should be some
infinite tuple $y$.

UNDER CONSTRUCTION


$\mathit{apply}(d)(x)$ to be the infinite tuple $y$ such
that $y_i$ is the join of applying $d_{j+1}$ to $x_j$, for all $j \geq
i$.

\begin{align*}
 \mathit{apply} &: D_{\infty} \to (D_{\infty} \to_c D_{\infty}) \\
 \mathit{apply}(d)(x) &=  y \\
  & \text{where } y_i = d_{i+1}(x_i) \sqcup p_i(d_{i+2}(x_{i+1})) \sqcup
    (p_i\circ p_{i+1})(d_{i+3}(x_{i+2}))
    \sqcup \cdots 
\end{align*}



The semantics of the $\lambda$-calculus in the $D_\infty$ model:
\begin{align*}
  \SEM{x}\rho &= \rho(x) \\
  \SEM{e_1 \APP e_2}\rho &= \mathit{apply}(\SEM{e_1}\rho)(\SEM{e_2}\rho) \\
  \SEM{\LAM{x} e}\rho &= \mathit{abstr}(\MLAM{d} \SEM{e}\rho(x:=d))
\end{align*}
