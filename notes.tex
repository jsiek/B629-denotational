...
\newcommand{\BIN}[0]{\ensuremath{\mathit{Bin}}}
\newcommand{\NAT}[0]{\ensuremath{\mathbb{N}}}
\newcommand{\BOOL}[0]{\ensuremath{\mathbb{B}}}
\newcommand{\VAR}[0]{\ensuremath{\mathbb{X}}}
\newcommand{\EXP}[0]{\ensuremath{\mathbb{E}}}
\newcommand{\CND}[0]{\ensuremath{\mathit{Cnd}}}
\newcommand{\CMD}[0]{\ensuremath{\mathit{Cmd}}}
\newcommand{\STORE}[0]{\ensuremath{\mathit{Store}}}
...
\begin{array}{lrl}
 \text{digit}& d ::=& 0 \mid 1 \\
 \text{binary numeral}& b \in \BIN ::=& \epsilon \mid bd  \\
 \text{expression}& e \in \EXP ::=& b \mid e + e \mid e \times e
\end{array}
\]
Semantics\\
...
Semantics 
\begin{align*}
\shortintertext{$P :  \CMD \to \SET{\NAT \times \NAT}$}
P\,c & = \{ (n,s'\,\mathtt{Z}) \mid (\{\mathtt{A}{\mapsto} n\},s') \in C\,c \}
\shortintertext{$ E :  \EXP \to \STORE \to \NAT$}
  E\,x\,s & = s\,x \\
    & \vdots 
\shortintertext{$ B : \CND \to \STORE \to \BOOL$}
  B\,(e_1 = e_2)\,s &= (E\,e_1\,s) =_{\NAT} (E\,e_2\,s) \\
  & \vdots 
...
with a partial function ($\NAT\pto\NAT$). Here we shall use the
alternate, but equivalent, approach of using a relation, a subset of
$\NAT \times \NAT$. A program is a command and the meaning of commands
...
Figure~\ref{fig:stlc} defines the simply-typed $\lambda$-calculus.
...
Type System\\
\begin{gather*}
  \inference{}{\Gamma \vdash n : \NAT}
  \quad
  \inference{\Gamma \vdash e_1 : \NAT & \Gamma \vdash e_2 : \NAT}
            {\Gamma \vdash e_1 \oplus e_2 : \NAT}
  \\[2ex]
...
Semantics
\begin{align*}
  E\,e\,\rho & : \SEM{A} &\text{if } \Gamma \vdash e : A \text{ and } \rho \models \Gamma\\[2ex]
  E\,n\,\rho &= n \\
