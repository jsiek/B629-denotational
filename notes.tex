...
$\lambda$ as a function that maps every element $d$ in its domain
$\SEM{A}$ to the element produced by the body $e$ of the $\lambda$,
with $d$ bound to parameter $x$. We interpret application as
mathematical function application. Regarding variables, the
environment $\rho$ is an association list mapping from variables to
values, so we use an auxilliary \textsf{lookup} function (like assq)
...
\intertext{$E : \prod (e : \EXP), \SEM{\Gamma} \to \SEM{A} \qquad \text{where } \Gamma \vdash e : A$}
  ...
  E\,x\,\rho &= \mathsf{lookup}\,\rho\,x \\
  E\,(\LAM{x\of A}e)\,\rho &= \{ (d,d') \mid d \in \SEM{A} \text{ and } d' = E\,e\,\ext{x}{d}{\rho} \} \\
...
In this section we construct a model for STLC that is built from the
expressions of STLC but modulo the equational theory. We then use this
model in the next section in the proof of completeness.

We define the equivalence class of expressions that are equal
according to the theory.
\[
  [e] \defeq \{ e' \mid \; \vdash e = e' \}
\]
We also define the equivalence classes of a given type.
Fix a type environment $\mathcal{G}$ with an infinite number of
distinct variable bindings for every type.
\[
...
\begin{definition}
A structure $(T,\cdot)$ is a \emph{pre-frame} when extensionality
holds, i.e. if two functions $f,g \in T (A \to B)$ map every argument
$a \in T\,A$ to equal results, $f \cdot a = g \cdot a$, then the two
...
\begin{definition}
A structure $(T,\cdot)$ is a \emph{frame} if $(T,\cdot)$ is a
pre-frame and $T$ is also a mapping on expressions that satisfies the
following equations
\begin{align*}
  T\,x\,\rho &= \rho(x) \\
  T\,(e_1\APP e_2)\,\rho &= (T\,e_1\,\rho) \cdot (T\,e_2\,\rho) \\
  (T\,(\LAM{x\of A} e_1)\,\rho) \cdot d &=
    T\,e_1\,\ext{x}{d}{\rho}
\end{align*}
\end{definition}
...
substitution $\sigma$ represents $\rho$ if it maps each variable to
the representative in $\rho$.
\[
\Gamma; \rho \vdash \sigma \defeq \forall x : A \in \Gamma,
   \rho(x) = [\sigma(x)]
\]

The meaning of expressions in the term model is defined as follows.
\[
  T\,e\,\rho \defeq [\sigma(e)]
  \text{ for some $\sigma$ s.t. } \Gamma;\rho \vdash \sigma
\]
...
\end{lemma}
\begin{proof} Let $\sigma$ be a substitution that represents $\rho$.
  \begin{itemize}
...
\begin{definition}
  ...
  \[
  (T,\cdot) \models e = e' \quad \defeq \quad \forall \rho,\;
  \rho \models \Gamma \implies T\,e\,\rho = T\,e'\,\rho
  \]
\end{definition}

\begin{lemma}[Frames are Sound]
  Suppose $(T,\cdot)$ is a frame.
  If $\vdash e = e'$, then $(T,\cdot) \models e = e'$
\end{lemma}


\begin{lemma}[Term Model is Sound and Complete]
    $\vdash e = e'$ iff $(T,\cdot) \models e = e'$.
\end{lemma}
\begin{proof}
  UNDER CONSTRUCTION
\end{proof}
