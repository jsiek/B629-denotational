...
\subsection{Interpreter and Adequacy of Semantics}
...
which pairs a $\lambda$ abstraction with its environment to ensure
that the free variables get their definitions from the lexical scope.
...
respect to the denotational semantics. That is, if the denotational
semantics says the program produces a result, does the interpreter
produce the same result? Alternatively, thinking of the interpreter as
an operational semantics that defines the CBV $\lambda$-calculus, the
same question becomes: is the denotational semantics adequate?

We prove this in two steps.  First we show that if the denotational
semantics says the result of a program should be an number $n$, then
the interpreter also produces $n$. The second part is to show that if
the interpreter produces an answer $n$, then the semantics agrees that
$n$ should be the answer.
...
\begin{align*}
  ...
  D_A &= \mathcal{P}(B_A) \\
   ...
   D_1 \cdot_E D_2 &= \{ d' \mid \exists D, D \subseteq D_2 \land (D,d') \in D_1 \} \\[1ex]
