...
\begin{proposition}[Partial functions are dcpos]
  Given any two sets $A$ and $B$, the partial order $(A \to B_\bot,
  \preceq)$, where the ordering is defined as subset inclusion on
  their graphs, is a dcpo.
\end{proposition}
\begin{proof}
  Let $S$ be a directed subset of $A \to B_\bot$.
  Let $r$ be the the union of the graphs of $S$.
  \[
    r = \bigcup \{ \mathrm{graph}(f) \mid f \in S\}
  \]
  Then least upper bound of $S$ is defined as follows
  \[
  (\bigsqcup S)(x) = 
    \begin{cases}
      y & \text{if } (x,y) \in r \\
